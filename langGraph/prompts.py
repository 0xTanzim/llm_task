...
def select_prompt_for_tools(
    selected_tools: list[object], user_input: str
) -> ChatPromptTemplate:
    """Select a prompt template that matches the currently selected toolset.

    This keeps the system instruction consistent with the router decision.
    """
    ...
    if "query_database" in tool_names or "get_database_schema" in tool_names:
        return DATABASE_PROMPT
    if (
        "execute_code" in tool_names
        or "explain_code" in tool_names
        or "suggest_refactor" in tool_names
    ):
        return CODE_PROMPT

    # Fallback to keyword-based selection.
    return dynamic_template_selector(user_input)
